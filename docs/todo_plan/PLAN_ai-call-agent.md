...
| BE2-2 | Agent 생성 (예약용 프롬프트) | 20분 |
| BE2-3 | Outbound Call API 호출 | 25분 |
| BE2-4 | 웹훅 엔드포인트 (POST /api/webhooks/elevenlabs) | 20분 |
| BE2-5 | 통화 결과 처리 & DB 업데이트 | 10분 |
...
| INT-3 | FE2 + BE2 | 통화 중 화면 → 상태 폴링 |
| INT-4 | FE2 + BE2 | 결과 화면 → 웹훅 결과 표시 |
| INT-5 | 전원 | End-to-End 플로우 테스트 |

**체크포인트**: 입력 → 확인 → 통화 → 결과 전체 플로우 동작
...
model Call {
  ...
  // 통화 결과
  status        String    @default("PENDING") // PENDING, CALLING, COMPLETED, FAILED
...
| 리스크 | 대응 |
|--------|------|
| ElevenLabs 연동 실패 | Mock 모드 준비 (BE2-5에서 가짜 결과 반환) |
| API 키 문제 | 사전에 테스트 필수 |
| 통합 시 충돌 | Git branch 전략 (main에 바로 push 금지) |
