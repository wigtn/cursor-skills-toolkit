...
- [ ] 요청 입력 UI
- [ ] 자연어 요청 파싱 (GPT-4)
- [ ] ElevenLabs Agent 설정 (예약/문의)
- [ ] ElevenLabs Outbound Call API 연동
- [ ] 통화 결과 웹훅 처리
- [ ] 결과 화면 UI
- [ ] 기본 알림 (웹 푸시)

**Deliverable**: 예약/문의 요청 → AI 통화 → 결과 확인
...
|--------|--------|-------------|
| 통화 성공률 | > 80% | 요청 → 목표 달성 비율 |
| 예약 완료율 | > 70% | 예약 요청 → 예약 확정 |
| 문의 답변율 | > 90% | 문의 요청 → 답변 수신 |
| 평균 통화 시간 | < 2분 | 연결 → 종료 |
